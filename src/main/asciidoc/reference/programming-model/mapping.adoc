...
[NOTE]
====
Consider batching saves when you have a collection of entities to be saved. The repository `save(Iterable<S> entities)` method will result in a single request sent to Neo4j, which is more performant than multiple `save(S entity)` calls.
====
