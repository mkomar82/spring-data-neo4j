...
To mark a field as indexed simply add the `@Index` annotation.  If your field must be unique add `@Index(unique=true)`.  You may add as many indexes or constraints as you like to your class.  If you annotate a field in a class that is part of an inheritance hierarchy then the index or constraint will only be added to that class' label.
...
SDN version 4.2 onwards supports derived finders in the form `findByPropertyNear`, and if the Neo4j database has a spatial index installed on that property, it will be employed. This derived finder format generates CYPHER that uses the built-in `distance` and `point` functions that are available in Neo4j version 3.0 onwards.

Previous versions of Spring Data Neo4j offered support for spatial queries using the `neo4j-spatial` library.
As of SDN 4, there is no dedicated support for the `neo4j-spatial` library.

If you are using a version of Neo4j prior to 3.x, a strategy similar to the full-text indexes being updated within
`AfterSaveEvents` can be employed to support Spatial Indexes. The http://neo4j-contrib.github.io/spatial/#spatial-server-plugin[Neo4j Spatial Plugin] exposes a REST API to interact with the library.
